                        ...
                        <div class="row">
                            <div class="col-sm-12 col-xs-12">
                                <form action="{{ route('deliveries.update',$delivery->id) }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="js-example-basic-single form-control" required name="employee_id" style="width: 100%;">
                                            ...
                                            @foreach ($employees as $employee)
                                                <option value="{{ $employee->id }}" {{ $employee->id == $delivery->employee_id ? 'selected' : ''}}>{{ $employee->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                            ...
                                            @if (\Auth::user()->isAdmin == 1)
                                                @foreach ($organizations as $organization)
                                                    <option value="{{ $organization->id }}"
                                                        {{ $organization->id == $delivery->organization_id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group">
                                        <label for="exampleInputEmail12">انتخاب کالا:</label>
                                        <select class="js-example-basic-single form-control" name="product_id" style="width: 100%;">
                                            <option value="">--هیچکدام--</option>
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}" {{ $product->id == $delivery->product_id ? 'selected' : ''}}>{{ $product->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputEmail12">انتخاب کالای دست دوم:</label>
                                        <select class="js-example-basic-single form-control" name="stock_id" style="width: 100%;">
                                            <option value="">--هیچکدام--</option>
                                            @foreach ($stocks as $stock)
                                                <option value="{{ $stock->id }}" {{ $stock->id == $delivery->stock_id ? 'selected' : ''}}>{{ $stock->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" value="{{ $delivery->deliverDate }}" name="deliverDate" required id="exampleInputEmail111">
                                    </div>
                                    <div class="form-group">
