                            ...
                            <div class="col-sm-12 col-xs-12">
                                <form action="{{ route('roles.update',$role->id) }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="title" value="{{ $role->title }}" required id="exampleInputEmail111">
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputEmail12">توضیح(فارسی):</label>
                                        <input type="text" class="form-control" name="description" value="{{ $role->description }}" required id="exampleInputEmail111">
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputEmail12">انتخاب سطح دسترسی:</label>
                                        <select class="js-example-basic-single form-control" name="permissions[]" multiple style="width: 100%;">
                                            @foreach ($permissions as $permission)
                                                <option value="{{ $permission->id }}"
                                                    {{ in_array($permission->id , $role->permissions->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $permission->description }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    <div class="checkbox checkbox-primary d-inline">
                                        <input type="checkbox" name="isActive" id="checkbox-p-1" {{ $role->isActive ? 'checked' : '' }}>
